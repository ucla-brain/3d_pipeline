...
def generate_bboxes(xJ, St, descendents_and_self, labels, ontology, outpath):
    ...
    for l in labels:
        ...
        for o in set(descendents_and_self[this_id]):
            g = gray_from_id[o]
...
def read_registration_values(reg_value_file):
    return np.load(reg_value_file, allow_pickle=True)[0]
...
def register():
    ...
    parser.add_argument("npy_file",type=pathlib.Path,help="Location of the output 2 file")
    ...
    assert os.path.isfile(npy_file), f"Registration Outfile is not file was not found: {npy_file}"
    ...
    print("Reading registration file")
    # now we want to register
    out2 = read_registration_values(npy_file)
    ...
    for l in ontology:
        ...
        # do marching cubes
        print('adding ',end='')
        for o in set(descendents_and_self[l]):
            print(f'{o},',end='')
            gray = gray_from_id[o]
